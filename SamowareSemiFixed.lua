...
---aaaaaa
ded.GetShiftingTickbase = function()return 0 end

/*
 SAMOWARE FOR LATEST ZXC
 Current things not fixed:
 Backtrack kinda
 Forwardtrack kinda
 Cvar
 shifting ticks
...
local function OpenMiscTab()
	...
	function threadsSlider:OnValueChanged(value)
		---samoware.SetThreadedSimulationThreads(value) tf is this
...
function UpdateFakeModel(model, angles)
	...
	if angles then
		...
		local lby = ded.GetCurrentLowerBodyYaw(ply)
		ang = Angle(0, lby, 0)
...
local function Unload()
	...
	hook_Remove("PostFrameStageNotify", "samoware")
...
local function UpdateSilentAngles(cmd)
	...
	silentangles.p = silentangles.p + dy * 0.020 * sensivity
	silentangles.y = silentangles.y + dx * -0.020 * sensivity
...
local GetServerTime = ded.GetCurTime
...
local function RemoveSpread(cmd, ang)
	...
	if class:StartWith("cw_") then
		...
		ded.SetContextVector(cmd, memevec)
...
function Aimbot(cmd)
	...
	if settings.HvH.LagFix then
		if isBackshoot then
			...
			if diff < 0.2 then
				...
				local tick = TIME_TO_TICKS(targetTime)
				...
				local tick = TIME_TO_TICKS(ded.GetServerTime(cmd))
				ded.SetCommandTick(cmd, tick - 1)
			end
		else
			local simTime = ded.GetSimulationTime(aimPly)

			if settings.HvH.Forwardtrack then
				local predTime = (ded.GetLatency(0) + ded.GetLatency(1)) * settings.HvH.ForwardtrackAmount
				simTime = simTime + predTime
			end

			local tick = TIME_TO_TICKS(simTime + 0)
			ded.SetCommandTick(cmd, tick)
		end
	end
	...
	if settings.Aimbot.PSilent and !weap:StartWith("cw_") and !weap:StartWith("arccw_") then
		...
		ded.SetContextVector(cmd, aimAngs:Forward())
	else
		cmd:SetViewAngles(aimAngs)
	end
...
AddHook("EntityFireBullets", function(ent, data)
    local bestPlayer = nil
    local bestDistance = math.huge

    for _, ply in ipairs(player.GetAll()) do
        local distance = ply:EyePos():DistToSqr(data.Src)
    ...
    if headPos then
        local bulletEnd = data.Src + data.Dir * 10000

        local nearestHeadPos = ClosestPointOnRay(
            headPos,
            data.Src,
            bulletEnd
        )
        ...
        if distanceToHead < 64 then
            local ang = -math.deg(
                math.atan2(headPosDiff.y, headPosDiff.x)
            ) + realangles.y

            ang = mNormalizeAng(ang)

            attacker.sw_anti_bruteforce_angle =
                ang > 0 and -1 or 1
        end
    end
end)
...
function YawAntiAim(cmd)
	...
	elseif mode == "LBY" then
		local angs = settings.Aimbot.Silent and silentangles or cmd:GetViewAngles()
		yaw = ded.GetCurrentLowerBodyYaw(me) + (bSendPacket and 180 or 0)
	elseif mode == "LBY Break" or mode == "LBY Break legit" then
		...
		if me:GetVelocity():Length2D() > 1 then
			...
			if !bSendPacket then
				local lbyTarget = ded.GetTargetLBY(me:EntIndex())
				if mabs(mNormalizeAng(lbyTarget - fakeangles.y)) < minDelta then
					...
					local lbyCurrent = ded.GetCurrentLowerBodyYaw(me)
	...
	elseif mode == "Hruxis" then
		...
		/*if !bSendPacket and chokes == 1 then --ticksUntilSend < 2 then
			print(bSendPacket)
			local lby = ded.GetCurrentLowerBodyYaw(me)
...
local function ShiftTickbase(amount)
	...
	*/
	...
	local ticksAllowed = ded.GetTicksAllowed()
	local maxShift = mmax(ticksAllowed - chokes, 0)
	amount = mClamp(amount or maxShift, 0, maxShift)

	print("SHIFT", amount, ticksAllowed)
	ded.SetTickbaseShift(amount)
end

local function RechargeShift()
	/*
	if ticksshiftedtotal == 0 then
		ded.SetOutSequenceNr(ded.GetOutSequenceNr() - MAX_TICKBASE_SHIFT)
	else
		ded.SetOutSequenceNr(ded.GetOutSequenceNr() - ticksshiftedtotal)
	end
	...
	*/
	...
	local ticksToRecharge = MAX_TICKBASE_SHIFT - ded.GetTicksAllowed()
	print("Charge", MAX_TICKBASE_SHIFT, ded.GetTicksAllowed())
	ded.SetTickbaseRecharge(ticksToRecharge)
end
...
function FakeEblan(cmd)
	...
	if keybinds.HvH.FakeEblanKey then
		if settings.HvH.FakeEblanMode == "Mega" then
			ded.SetOutSequenceNr(ded.GetOutSequenceNr() + mrandom(0, settings.HvH["FakeEblan strength"]))
		elseif settings.HvH.FakeEblanMode == "Slippery" then
			ded.SetOutSequenceNr(ded.GetOutSequenceNr() + fakeeblan_factor * settings.HvH["FakeEblan strength"] )
		elseif settings.HvH.FakeEblanMode == "Airstuck" then
			ded.SetOutSequenceNr(ded.GetOutSequenceNr() + 2500)
		elseif settings.HvH.FakeEblanMode == "M9K" then
			...
			ded.SetOutSequenceNr(ded.GetOutSequenceNr() + 5000 * settings.HvH["FakeEblan strength"])
		end
	end

	local curack = ded.GetOutSequenceNrAck()
	local diff = curack - prevack
	if diff == 0 then
		numerrors = numerrors + 1
		if numerrors > 33 then
			print(("[samoware] Too much lost packets, fixing (delta %d)"):format(ded.GetOutSequenceNr() - ded.GetOutSequenceNrAck()))
			ded.SetOutSequenceNr(curack)
...
do
	...
	local function ChooseBestAngularVelocity(angularVelocities, moveDatas, simTime, pos, speed2d)
		...
		ded.StartThreadedSimulation(moveDatas)

		-- Wait for aimbot
		coroutine.yield()

		local results = ded.FinishThreadedSimulation(#moveDatas)
...
AddHook("EntityFireBullets", function(ent, data)
    ...
    bulletImpacts[#bulletImpacts + 1] = {
        ...
        startPos = data.Src,
        endPos = data.Src + data.Dir * (data.Distance or 56756),
        hitbox = nil,
    }

    if bestDistance < 128 and IsValid(bestPlayer) then
...
AddHook("HUDPaint", function()
	...
	local rl, fk, lby = Angle(0, realangles.y, 0), Angle(0, fakeangles.y, 0), Angle(0, ded.GetCurrentLowerBodyYaw(me), 0)
...
do
	for _, ply in ipairs(player.GetAll()) do
		...
		ply.sw_cur_simtime = ded.GetSimulationTime(ply)
...
AddHook("player_connect_client", function(ply)
	...
	ply.sw_cur_simtime = ded.GetSimulationTime(v)
	ply.sw_prev_simtime = 0
...
AddHook("Think", function()
	...
	if settings.HvH.Cvar3 then
		...
		if settings.HvH.NoInterp then
			...
			ded.SetEnableInterpolation(false)
		else
			c_cl_extrapolate:SetValue(1)
			ded.SetEnableInterpolation(true)
		end
	end
...
AddHook("CreateMove", function(cmd)
	local shiftingTickbase = ded.GetShiftingTickbase()
	if settings.Aimbot.Silent then
	...
	-- Reset cl_interp
	---if ded.GetManipulateInterp() then
		ded.SetInterpoloationAmount(0)
	---end
	...
	if settings.HvH.Enginepred then ded.StartPrediction(cmd) end
	...
	if aimbotsucc then
		...
		realangles = angles
		fakeangles = angles
	elseif !(cmd:KeyDown(IN_ATTACK) and CanShoot(cmd)) and !cmd:KeyDown(IN_USE) then
	...
	if settings.HvH.FakeEblan then
		FakeEblan(cmd)
	end
	...
	if settings.HvH["Arm breaker"] and bSendPacket then
	...
	if settings.HvH.Enginepred then ded.FinishPrediction() end
	...
	if settings.Misc["SimplAC safe mode"] then
		...
		cmd:SetMouseX(diff.y / -0.020)
		cmd:SetMouseY(diff.p / 0.020)
	end
